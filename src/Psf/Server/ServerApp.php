...
class ServerApp
{
    ...
    private static function initObject()
    {
        ...
        $definitions = ServerManager::$config['beans'] ?? [];
    ...
    {
        $routeMaps = ServerManager::$config['routeMap'] ?? [];
        $routeCollector = Container::getInstance()->getSingleton(RouteCollector::class);
        foreach ($routeMaps as $routeName => $routeData) {
            list($httpMethod, $route, $handler) = $routeData;
